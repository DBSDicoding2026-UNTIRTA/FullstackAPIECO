  ...
  return (
    <section className="rounded-[2rem] border border-emerald-100 bg-white p-6 shadow-[0_24px_64px_-36px_rgba(16,185,129,0.35)] dark:border-emerald-900/60 dark:bg-slate-900">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-yellow-100 px-4 py-2 text-sm font-black text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-200">
            <Sparkles className="h-4 w-4" />
            +{quiz.points} XP
          </div>

          <h2 className="mt-5 text-2xl font-black leading-tight text-slate-900 dark:text-white">
            {quiz.question}
          </h2>
        </div>
      </div>
            ...
            <button
              ...
              className={`flex items-center gap-4 rounded-[1.25rem] border p-4 text-left transition ${
                isSelected
                  ...
                  : "border-slate-200 bg-white text-slate-700 hover:border-emerald-300 hover:bg-emerald-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-emerald-950/20"
      ...
      >
        {submitting ? (
          <>
            ...
            {t("quiz.actions.checking")}
          </>
        ) : (
          t("quiz.actions.checkAnswer")
        )}
      </button>
    </section>
  );
